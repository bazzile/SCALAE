...
def save_sample(lod2batch, tracker, sample, sample_pop, samplez, x, logger, model, cfg, encoder_optimizer, decoder_optimizer):
    ...
    with torch.no_grad():
        ...
        sample_pop = sample_pop[:lod2batch.get_per_GPU_batch_size()]
        ...
        while sample_in.shape[2] > needed_resolution:
            ...
            sample_pop = F.avg_pool2d(sample_pop, 2, 2)
        assert sample_in.shape[2] == needed_resolution, sample_in.shape
        assert sample_pop.shape[2] == needed_resolution, sample_pop.shape
        ...
        Z, _ = model.encode(torch.cat((sample_in, sample_pop), 1),
                            lod2batch.lod, blend_factor)
        ...
        rec1 = model.decoder(Z, sample_pop, lod2batch.lod, blend_factor, noise=False)
        rec2 = model.decoder(Z, sample_pop, lod2batch.lod, blend_factor, noise=True)
        ...
        g_rec = model.decoder(Z, sample_pop, lod2batch.lod, blend_factor, noise=True)
        # g_rec = F.interpolate(g_rec, sample.shape[2])

        resultsample = torch.cat([sample_pop.repeat(1, 3, 1, 1), sample_in, rec2, rec1, g_rec], dim=0)
        ...
        def save_pic(x_rec):
            ...
            f = os.path.join(cfg.OUTPUT_DIR, f'sample_{(lod2batch.current_epoch + 1):0>4d}_{(lod2batch.iteration // 1000):0>4d}.jpg')
            print("Saved to %s" % f)
...
def train(cfg, logger, local_rank, world_size, distributed):
    ...
    if cfg.DATASET.SAMPLES_PATH:
        ...
        sample, sample_pop, _, _ = next(make_dataloader(cfg, logger, dataset, 32, local_rank))
        sample = (sample / 127.5 - 1.)
        sample_pop = (sample_pop / 127.5 - 1.)
    ...
    for epoch in range(scheduler.start_epoch(), cfg.TRAIN.TRAIN_EPOCHS):
        ...
        for x_orig, pop_orig, pop_fake_1, pop_fake_2 in tqdm(batches):
            ...
            with torch.no_grad():
                ...
                if need_permute:
                    ...
                    pop_orig = pop_orig.permute(0, 3, 1, 2)
                x_orig = (x_orig / 127.5 - 1.)
                pop_orig = (pop_orig / 127.5 - 1.)
                pop_fake_1 = (pop_fake_1 / 127.5 - 1.)
                pop_fake_2 = (pop_fake_2 / 127.5 - 1.)
                ...
                if lod2batch.in_transition:
                    ...
                    pop_prev = F.avg_pool2d(pop_orig, 2, 2)
                    pop_prev_2x = F.interpolate(pop_prev, needed_resolution)
                    pop = pop * blend_factor + pop_prev_2x * (1.0 - blend_factor)
            ...
            encoder_optimizer.zero_grad()
            decoder_optimizer.zero_grad()
            lae = model(x, pop_fake_2, lod2batch.lod, blend_factor, d_train=True, ae=True)
        ...
        if local_rank == 0:
            ...
            save_sample(lod2batch, tracker, sample, sample_pop, samplez, x, logger, model_s, cfg, encoder_optimizer, decoder_optimizer)
